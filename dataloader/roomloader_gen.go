...
type RoomLoaderConfig struct {
	...
	Fetch func(keys []int) ([]*model.Room, []error)
...
func (l *RoomLoader) Load(key int) (*model.Room, error) {
...
func (l *RoomLoader) LoadThunk(key int) func() (*model.Room, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Room, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *RoomLoader) LoadAll(keys []int) ([]*model.Room, []error) {
	results := make([]func() (*model.Room, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	rooms := make([]*model.Room, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *RoomLoader) LoadAllThunk(keys []int) func() ([]*model.Room, []error) {
	results := make([]func() (*model.Room, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *RoomLoader) Prime(key int, value *model.Room) bool {
	l.mu.Lock()
	var found bool
...
func (l *RoomLoader) Clear(key int) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *RoomLoader) unsafeSet(key int, value *model.Room) {
	if l.cache == nil {
		l.cache = map[int]*model.Room{}
	}
	l.cache[key] = value
}
...
func (b *roomLoaderBatch) keyIndex(l *RoomLoader, key int) int {
